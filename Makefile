...
jetson:
	docker buildx build --platform linux/arm64 --target=frigate-jetson --tag $(IMAGE_REPO):$(VERSION)-$(COMMIT_HASH)-jetson $(JETSON_ARGS) .
...
push: build
	...
	docker buildx build --push --platform linux/arm64 --target=frigate-jetson --tag $(IMAGE_REPO):$(GITHUB_REF_NAME)-$(COMMIT_HASH)-jetson $(JETSON_ARGS) .
